...
class SemGCN(Model):
    ...
    def make_batch(self, shuffle=True):
        ...
        for semantic in self.semantic_list:
            ...
            for line in open('./semantic_info/{}.txt'.format(semantic), encoding='utf-8', errors='ignore'):
                ...
                ele['Edges'] = [[i, j, self.lbl2id[semantic]] for i, j in
                                itertools.permutations(range(len(ele['Words'])), 2)]
                ele['Cur_len'] = len(ele['Words'])
                ele['Edges'] = [[e[0], e[1], e[2]] for e in ele['Edges'] if
                                ele['Words'][e[0]] != 0 and ele['Words'][e[1]] != 0]
                batch.append(ele)
    ...
    def load_data(self):
        ...
        self.rej_prob = {_id: (1 - self.p.subsample / rel_freq[_id]) - np.sqrt(self.p.subsample / rel_freq[_id]) for _id
                         in self.id2freq}
    ...
    def gcnLayer(self, gcn_in, in_dim, gcn_dim, batch_size, max_nodes, max_labels, adj_mat, w_gating=True, num_layers=1,
                 name="GCN"):
        """
            ...
            with tf.name_scope('%s-%d' % (name, layer)):
                ...
                for lbl in range(max_labels):
                    ...
                    with tf.variable_scope('label-%d_name-%s_layer-%d' % (lbl, name, layer)) as scope:
                        w_in = tf.get_variable('w_in', initializer=tf.eye(in_dim), trainable=True,
                                               regularizer=self.regularizer)
                        w_out = tf.get_variable('w_out', initializer=tf.eye(in_dim), trainable=True,
                                                regularizer=self.regularizer)
                        b_in = tf.get_variable('b_in', [1, gcn_dim], trainable=True,
                                               initializer=tf.constant_initializer(0.0), regularizer=self.regularizer)
                        b_out = tf.get_variable('b_out', [1, gcn_dim], trainable=True,
                                                initializer=tf.constant_initializer(0.0), regularizer=self.regularizer)
                        ...
                        if w_gating:
                            w_gin = tf.get_variable('w_gin', [in_dim, 1],
                                                    ...
                                                    regularizer=self.regularizer)
                            b_gin = tf.get_variable('b_gin', [1], initializer=tf.constant_initializer(0.0),
                                                    regularizer=self.regularizer)
                    ...
                    with tf.name_scope('in_arcs-%s_name-%s_layer-%d' % (lbl, name, layer)):
                        ...
                        if w_gating:
                            inp_gin = tf.tensordot(gcn_in, tf.sigmoid(w_gin), axes=[2, 0]) + tf.expand_dims(b_gin,
                                                                                                            axis=0)
    ...
    def add_model(self):
        ...
        with tf.variable_scope('Embed_mat'):
            embed_init = getEmbeddings(self.p.embed_loc, [self.id2voc[i] for i in range(len(self.voc2id))],
                                       self.p.embed_dim)
            _wrd_embed = tf.get_variable('embed_matrix', initializer=embed_init, trainable=True,
                                         regularizer=self.regularizer)
    ...
    def run_epoch(self, sess, epoch, shuffle=True):
        ...
        for step, batch in enumerate(self.getBatches(shuffle)):
            ...
            if (step + 1) % 10 == 0:
                self.logger.info('E:{} (Sents: {}/{} [{}]): Train Loss \t{:.5}\t{}\t{:.5}'.format(epoch, self.sent_num,
                                                                                                  self.p.total_sents,
                                                                                                  round(
                                                                                                      self.sent_num / self.p.total_sents * 100,
                                                                                                      1),
                                                                                                  np.mean(losses),
                                                                                                  self.p.name,
                                                                                                  self.best_int_avg))
