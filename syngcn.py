...
class SynGCN(Model):
    ...
    def getBatches(self, shuffle=True):
        ...
        while True:
            ...
            eph_ovr = self.lib.getBatch(self.edges_addr, self.wrds_addr, self.negs_addr, self.samp_addr, self.elen_addr,
                                        self.wlen_addr,
                                        self.p.win_size, self.p.num_neg, self.p.batch_size,
                                        ctypes.c_float(self.p.sample))
            if eph_ovr == 1: break
            yield {'edges': self.edges, 'wrds': self.wrds, 'negs': self.negs, 'sample': self.samp, 'elen': self.elen,
                   'wlen': self.wlen}
    ...
    def load_data(self):
        ...
        self.de2id = read_mappings(os.path.join(base_dir, 'de2id.txt'))
                ...
                for lbl in range(max_labels):
                    with tf.variable_scope('label-%d_name-%s_layer-%d' % (lbl, name, layer)) as scope:
                        ...
                        w_out = tf.get_variable('w_out', [in_dim, gcn_dim],
                                                initializer=tf.contrib.layers.xavier_initializer(),
                                                regularizer=self.regularizer)
                        b_in = tf.get_variable('b_in', [1, gcn_dim], initializer=tf.constant_initializer(0.0),
                                               regularizer=self.regularizer)
    ...
    def run_epoch(self, sess, epoch, shuffle=True):
        ...
        for step, batch in enumerate(self.getBatches(shuffle)):
            ...
            if (step + 1) % 10 == 0:
                self.logger.info('E:{} (Sents: {}/{} [{}]): Train Loss \t{:.5}\t{}\t{:.5}'.format(
                    epoch, cnt, self.p.total_sents, round(cnt / self.p.total_sents * 100, 1), np.mean(losses),
                    self.p.name, self.best_int_avg))
                en = time.time()
                if (en - st) >= 3600:
